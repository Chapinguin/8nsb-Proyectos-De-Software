...
try {
    ...
    if (!$stmtExiste->fetch()) {
        jsonResponse(404, [
            "ok" => false,
            "message" => "No se encontró el consultorio a eliminar"
        ]);
    }
    ...
    $mensaje = "Error al eliminar consultorio";

    if (
        stripos($e->getMessage(), "foreign key") !== false ||
        stripos($e->getMessage(), "constraint") !== false
    ) {
        $mensaje = "No se puede eliminar el consultorio porque está relacionado con otras tablas";
    }
